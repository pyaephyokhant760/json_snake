<?php
require_once 'includes/functions.php';

// JSON ဖိုင်လမ်းကြောင်း
$dataFile = './data/user.json';

// JSON ဒေတာကိုဖတ်ခြင်း
$users = readJSON($dataFile);
?>

<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Snake</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">

</head>

<body>
    <div class="container-fluid">
        <div class="offset-3 col-6">
            <?php foreach ($users as $user): ?>
                <div class="card mb-3">
                    <img src="<?php echo $user['image']; ?>" class="card-img-top" alt="...">
                    <div class="card-body">
                        <h5 class="card-title"><?php echo $user['MMName']; ?></h5>
                        <?php if (isset($user['EngName'])): ?>
                            <small class="card-text"><?php echo $user['EngName']; ?></small>
                        <?php endif; ?>
                        <p class="card-text"><small class="text-body-secondary"><?php echo substr($user['Detail'],0,110); ?></small></p>

                        <a href="detail.php?id=<?php echo $user['Id']; ?>" class="btn btn-primary">Detail</a>
                    </div>
                </div>

            <?php endforeach; ?>

        </div>
    </div>
</body>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous"></script>

</html>